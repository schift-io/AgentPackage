...
- **포맷 A — 라벨 6섹션**: 구조물 전반(포스터·키아트·인포그래픽·도감·카드뉴스·만화 등). `Scene / Camera / Lighting / Color grading / Texture/Medium / Text-in-image` 순서의 구조화된 절을 사용하고, 마지막에 `AR x:y` 토큰을 하나만 둔다.
- **포맷 B — 화보 플랫 콤마형 단문**: 라벨 없이 콤마 한 문단, 350~450자. 기본 AR은 `2:3`이며, 슬롯 12종 순서와 Tier-2 문구는 `references/editorial-hwabo.md`를 따른다.
- 라우팅: 단독 인물 화보·글래머 에디토리얼은 C1·Format B, 그 외 C1~C12는 Format A를 기본으로 한다. C11은 상류 규칙에 따라 영문 Format A를 사용한다.

## 라우터 위임 규칙

| 요청 신호 | 선택 | 읽을 파일 |
|---|---|---|
| 단독 인물 화보·에디토리얼 | C1·Format B | `references/editorial-hwabo.md` 및 필요 시 `references/style-taxonomy.md` |
| 타이포 포스터·글자가 곧 이미지 | TP1~TP14 | `references/typo-poster-router.md` → `references/typo-poster/TPn-*.md` 중 하나 |
| 홍보판촉물·브랜드 포스터·디자인된 포스터 | P1~P8 | `references/promo-router.md` → `references/promo/Pn-*.md` 중 하나 |
| 포스터·키아트·인포그래픽·카드뉴스·만화·도감·아이콘·뷰티·캠페인·목업 | C2~C11 | 이 문서의 해당 카테고리 |
| 프레젠테이션·슬라이드 덱 | C12 | 이 문서의 C12 |
| 무드(“있어 보이게”·“럭셔리”·“영화처럼”) | 룩 L1~L9 | `references/look-presets.md`에서 하나 |
| 시안 다변화·양산 컨셉·차별화 | M/R/X/T축 | `references/concept-axes.md`에서 하나 |
| 글자 배치·폰트·그리드·밀집 텍스트 | 보조 규칙 | `references/typography-layout.md` |

라우터(P/TP)는 패턴 하나를 선택한 뒤 해당 패턴 파일 하나만 로드한다. 카테고리 규칙과 라우터 규칙이 동시에 걸리면 라우터 패턴을 먼저 적용하고, 이 문서의 카테고리 필수 디테일을 보완한다.

## 멀티모델 프로파일

카테고리 데이터는 모델별 축 선택과 분리한다. 컴파일 시 대상 모델의 축 순서·팔레트·카메라 언어·최대 절 수는 `references/model-profiles.md`를 참조한다. 기본 `gpt-image-2`는 `Scene, Camera, Lighting, Color grading, Texture, Text-in-image`를 사용하며, `higgsfield`와 `z-image-turbo`는 각각 해당 프로파일의 축을 따른다.

## AR 토큰과 `api_size` 분리

프롬프트 끝에는 의도한 비율을 나타내는 `AR x:y` 토큰만 둔다. API 호출 크기는 별도 `api_size` 필드로 전달하며 두 값을 하나의 브래킷 토큰으로 합치지 않는다. 상류의 사이즈 락과 별개로, 현재 팩의 API 브리지는 지원 캔버스에 맞춰 다음처럼 매핑한다.

| AR | `api_size` | AR | `api_size` |
|---|---|---|---|
| `1:1` | `1024x1024` | `16:9` | `1536x1024` |
| `2:3` / `3:4` / `4:5` | `1024x1536` | `9:16` | `1024x1536` |
| `3:2` / `4:3` | `1536x1024` | `admob-banner` / `admob-large` / `admob-leader` | `1536x1024` |
| `admob-mrect` | `1024x1024` | 기타 미지원 캔버스 | `auto` |

AdMob처럼 API 캔버스와 정확히 일치하지 않는 비율도 `AR`에는 실제 의도 비율을 유지하고, `api_size`에는 가장 가까운 지원 캔버스를 기록한다. `auto`는 프롬프트의 AR 토큰이 아니라 API 필드에서만 사용한다. 상류 예시가 `1792x1024`, `1024x1792`, `2048x2048`을 권장하는 경우에도 이 팩의 실제 호출에서는 위 API 브리지 매핑을 우선한다.

## 출처 및 라이선스

C1~C12 카테고리 패턴, 공통 시각 DNA, 무드, Format A/B 맥락, 라우터 위임 규칙은 `kimsh-1/gongnyang-prompt-kit` V2.4의 한국어 원본을 반영했다. 상류 프로젝트의 라이선스는 MIT이며 저작자는 `kimsh-1`이다. 이 팩의 멀티모델 프로파일과 `api_size` 브리지는 팩 고유 확장이다.
