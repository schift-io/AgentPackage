...
명령:
  init   <name> [--display-name] [--description] [--output DIR]
                                                  최소 동작 `.agent` 스캐폴드 생성 + 자동 lint
...
def _init_agent_md(display_name: str, description: str) -> str:
    return (
        f"# {display_name}\n\n"
        f"{description}\n\n"
        "## 동작 원칙\n"
        "1. TODO: 이 에이전트가 실제로 해야 할 일을 적는다.\n"
        "2. 사실을 지어내지 않는다. 확인되지 않은 내용은 [확인 필요]로 표시한다.\n"
    )


def _init_apm_yml(name: str, display_name: str, version: str, description: str) -> str:
    try:
        import yaml
    except ImportError:
        sys.exit("apm-kit requires PyYAML: pip install pyyaml")
...
def _init_pack_json(
    name: str, display_name: str, version: str, description: str, agent_md: str
) -> dict[str, Any]:
    return {
        "schema_version": 1,
        "agent_id": name,
        ...
        "description": description,
        "visibility": "private",
        "agents_md_ref": f"{name}.agent/agent.md",
        "agents_md_content": agent_md,
        "document_hierarchy": {
            "kind": "document",
            "template_id": f"{name}_v1",
            "title": display_name,
            "sections": [
                {"id": "1", "title": "본문", "prompt": "요청받은 내용을 작성한다."}
            ],
            "research_queries": [],
        },
        "tools": [],
        ...
        "manifest_overrides": {
            ...
            "version": version,
            "visibility": "private",
            "agents": [{"id": f"{name}.agent/agent.md", "path": "agent.md"}],
            "skills": [],
        },
    }
...
def cmd_init(args: argparse.Namespace) -> int:
    """`blank.agent` 구조를 최소화한 스캐폴드 — 만들자마자 `lint`/`build` 가 통과한다.

    sub_agent_plan·skill_manifests 같은 고급 필드는 안 채운다(빈 배열/생략). 필요한
    건 저작자가 직접 채운다 — init 은 "0에서 시작하지 않게" 하는 게 목적이지 완성된
    팩을 만드는 게 아니다.
    """
    name = args.name
    if not NAME_RE.fullmatch(name):
        print(
            f"✗  init: 이름은 소문자/숫자/하이픈만 허용한다(agent_id 로 그대로 쓰인다): {name!r}",
            file=sys.stderr,
        )
        return 1
    ...
    description = args.description or f"{display_name} 에이전트."

    output = Path(args.output) if args.output else Path.cwd() / f"{name}.agent"
    if output.exists():
        print(f"✗  init: 이미 존재하는 경로라 덮어쓰지 않는다: {_display_path(output)}", file=sys.stderr)
        return 1

    agent_md = _init_agent_md(display_name, description)
    apm_yml = _init_apm_yml(name, display_name, version, description)
    pack_json = _init_pack_json(name, display_name, version, description, agent_md)

    output.mkdir(parents=True)
    (output / "agent.md").write_text(agent_md, encoding="utf-8")
    (output / "apm.yml").write_text(apm_yml, encoding="utf-8")
    (output / "pack.json").write_text(
        json.dumps(pack_json, ensure_ascii=False, indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )
    print(f"✓  init: {_display_path(output)} 생성됨 (agent.md, apm.yml, pack.json)")

    # 만들자마자 lint 로 자가검증 — 스캐폴드가 애초에 깨져서 나가면 init 의 존재
    # 이유가 없다. lint 는 pack.json 이 아니라 apm.yml/agent.md 를 보므로 여기서
    # 함께 검증해야 pack.json 쪽 결함(예: 유효하지 않은 JSON)도 놓치지 않는다.
    lint_args = argparse.Namespace(
        pack=name,
        packs_dir=str(output.parent),
        identity_patterns=str(DEFAULT_IDENTITY_PATTERNS),
    )
...
def main() -> int:
    ...
    p_init = sub.add_parser("init", help="scaffold a new .agent dir that lints/builds immediately")
    p_init.add_argument("name", help="agent_id — 소문자/숫자/하이픈만")
    p_init.add_argument("--display-name", help="사람이 읽는 이름 (기본: name)")
    p_init.add_argument("--description", help="한 줄 설명 (기본: '<display-name> 에이전트.')")
    p_init.add_argument(
        "--output", help="생성 경로 (기본: 현재 디렉터리의 <name>.agent)"
    )
